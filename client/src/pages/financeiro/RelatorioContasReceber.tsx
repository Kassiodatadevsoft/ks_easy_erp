...
type Linha = {
  documento: string | null; parcela: string; cliente: string | null; emissao: string; vencimento: string;
  dataRecebimento: string | null; formaPagamento: string | null; contaFinanceira: string | null;
  centroCusto: string | null; naturezaFinanceira: string | null; vendedor: string | null;
  qtdAnexos?: number;
  valorOriginal: number; juros: number; multa: number; desconto: number; valorRecebido: number; saldo: number; situacao: string;
};
type Resultado = {
  dados: Linha[];
  resumo: Record<string, number>;
  totais: Record<string, { nome: string; valor: number }[]>;
};
...
export default function RelatorioContasReceber() {
  ...
  const [situacao, setSituacao] = useState<"ABERTO" | "RECEBIDO" | "VENCIDO" | "TODOS">("TODOS");
  const [resultado, setResultado] = useState<Resultado | null>(null);

  const { data: filtros } = trpc.financeiroRelatorios.filtrosContasReceber.useQuery();
  const params = {
  ...
  async function exportarExcel() {
    ...
    const rows = [
      ...
      ...filtroTexto.map((f) => [f]),
      [],
      ["Documento","Parcela","Cliente","Emissao","Vencimento","Data Recebimento","Forma de Pagamento","Conta Financeira","Anexo","Valor Original","Juros","Multa","Desconto","Valor Recebido","Saldo","Situacao"],
      ...data.dados.map((l) => [l.documento ?? "", l.parcela, l.cliente ?? "", br(l.emissao), br(l.vencimento), br(l.dataRecebimento), l.formaPagamento ?? "", l.contaFinanceira ?? "", Number(l.qtdAnexos ?? 0) > 0 ? "Sim" : "Nao", l.valorOriginal, l.juros, l.multa, l.desconto, l.valorRecebido, l.saldo, l.situacao]),
    ];
    baixar(`relatorio_contas_receber_${ymd()}.xls`, new Blob([`<html><meta charset="utf-8"/><body><table>${rows.map((r) => `<tr>${r.map((c) => `<td>${esc(c)}</td>`).join("")}</tr>`).join("")}</table></body></html>`], { type: "application/vnd.ms-excel;charset=utf-8" }));
  }
  ...
  const cards = [
    ...
    ["Total Recebido", resumo.totalRecebido ?? 0, true],
    ["Total Vencido", resumo.totalVencido ?? 0, true],
    ["Valor a Receber Hoje", resumo.valorReceberHoje ?? 0, true],
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-3 sm:p-6">
      <div><h1 className="text-2xl font-bold">Contas a Receber</h1><p className="text-sm text-muted-foreground">Titulos, recebimentos, vencidos e totais dinamicos da empresa logada.</p></div>
      <Card><CardContent className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="space-y-1"><Label>Cliente</Label><Select value={guidCliente} onValueChange={setGuidCliente}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="todos">Todos</SelectItem>{options(filtros?.clientes, "guidCliente", "nome")}</SelectContent></Select></div>
        <div className="space-y-1"><Label>Vendedor</Label><Select value={guidVendedor} onValueChange={setGuidVendedor}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="todos">Todos</SelectItem>{options(filtros?.vendedores, "guidVendedor", "nome")}</SelectContent></Select></div>
        ...
        <div className="space-y-1"><Label>Situação</Label><Select value={situacao} onValueChange={(v) => setSituacao(v as typeof situacao)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ABERTO">Aberto</SelectItem><SelectItem value="RECEBIDO">Recebido</SelectItem><SelectItem value="VENCIDO">Vencido</SelectItem><SelectItem value="TODOS">Todos</SelectItem></SelectContent></Select></div>
