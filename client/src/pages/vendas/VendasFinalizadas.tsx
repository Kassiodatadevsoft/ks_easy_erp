...
export default function VendasFinalizadas() {
  ...
  function htmlImpressao(payload: { modelo: string; empresa: typeof user; dados: VendaDetalhe }) {
    const { modelo, dados } = payload;
    const bobina = modelo === "BOBINA";
    const venda = dados.venda;
    const itens = dados.itens.map((i) => bobina
      ? `<tr><td>${esc(i.produto)}<br/><small>${Number(i.quantidade)} x ${esc(moeda(i.valorUnitario))}</small></td><td class="num">${esc(moeda(i.valorTotal))}</td></tr>`
      : `<tr><td>${i.item}</td><td>${esc(i.produto)}</td><td class="num">${Number(i.quantidade).toLocaleString("pt-BR")}</td><td class="num">${esc(moeda(i.valorUnitario))}</td><td class="num">${esc(moeda(i.desconto))}</td><td class="num">${esc(moeda(i.valorTotal))}</td></tr>`).join("");
  ...
  async function imprimir(venda: Venda, modelo: "a4" | "bobina") {
    try {
      const data = await api<{ modelo: string; empresa: typeof user; dados: VendaDetalhe }>(`/api/vendas-gerencial/${venda.guidVenda}/impressao-${modelo}`);
      const win = window.open("", "_blank", "width=1100,height=800");
      if (!win) return toast.error("Não foi possível abrir a impressão.");
      win.document.open();
      win.document.write(htmlImpressao(data));
      win.document.close();
      win.focus();
