...
export const fornecedoresRouter = router({
  ...
  /**
   * Listar fornecedores da empresa logada com filtros e paginação
   */
  listar: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      // Filtro principal: CADFORNECEDOR = 1
      let where = `WHERE c.CADFORNECEDOR = 1 AND c.GUIDENTIDADE = @GUIDENTIDADE`;
  ...
  /**
   * Buscar um fornecedor pelo GUIDPESSOA
   */
  ...
  /**
   * Criar novo fornecedor — CADFORNECEDOR = 1 fixo, CADCLIENTE vem do input
   */
  criar: publicProcedure
    .input(z.object({
      ...
      cadCliente: z.boolean().default(false),   // "Também é Cliente"
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await querySql(
        `INSERT INTO KS0002.KS00001 (
          ...
          SITUACAO, CADFORNECEDOR, CADCLIENTE, MANTERPROMOCOES,
          CADUSUARIO, CONSTASPC,
          OBSERVACAO,
          ...
          @CEP, @ENDERECO, @NUMERO, @COMPLEMENTO, @BAIRRO, @CODCIDADE,
          @LIMITECOMPRA, @DIAVENCIMENTO,
          @SITUACAO, 1, @CADCLIENTE, @MANTERPROMOCOES,
          @CADUSUARIO, @CONSTASPC,
          @OBSERVACAO,
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await querySql(
        `UPDATE KS0002.KS00001 SET
          ...
          CADUSUARIO = @CADUSUARIO, CADCLIENTE = @CADCLIENTE,
